 ...
 * 快取鍵前綴，含**版本段**。
 *
 * 版本段是 2026-09-02（身分鑰匙第三階段）加的。報到 QR 裡的身分從員編換成內部碼，
 * 而手機上存的是換之前簽出來的碼。沒有版本段的話，圖文選單那顆常駐鈕走 `:auto` 鍵、
 * 又適用「沒帶 v 就永不重抓」，會一直拿舊碼畫 QR ——**畫面完全正常，到現場才掃不進去**。
 * 版本一 bump，舊鍵整批讀不到，前端自己重抓一次就拿到新碼；同仁不必做任何事。
 *
 * ⚠️ 舊鍵的內容留在 localStorage 裡不會被清掉（讀不到 ≠ 刪掉）。那是刻意的：
 * 清理要列舉整個 localStorage，而這裡拿不到「哪些鍵是我們的」以外的保證。
 * 幾百 bytes 的殘留換掉一次全域掃描，划算。
 *
 * ═══════════════════════════════════════════════════════════════════════
 * 🔴 **v2 → v3（2026-09-11，報到碼加 `CHK2` 版本前綴）**
 * ═══════════════════════════════════════════════════════════════════════
 *
 * 上一版留了一句「下次再換身分鑰匙時要 bump 成 v3，不要重用 v2」。**這就是那一次。**
 *
 * 這次非 bump 不可的理由比上次更硬：掃描站現在會對舊碼說「**請重新開啟通行證**」。
 * 而「重新開啟通行證」走的正是 `:auto` 鍵，適用「沒帶 v 就永不重抓」
 * ⇒ **不 bump 的話，他照做了、卻拿到同一張舊碼、再掃一次還是同一句話**，
 * 而畫面從頭到尾沒有任何異狀。
 *
 * ⚠️ 這不是「舊碼失效」那半塊的附帶項，是「**畫面上那句指示是不是真的**」那半塊。
 * 假的處置比沒有處置更貴：它看起來像系統在幫忙，實際上把人留在原地。
 *
 * 下次再換身分鑰匙時要 bump 成 v4，不要重用 v3——而且要跟 `CHK` 前綴同一批。
 */
var PASS_CK = 'jdcPass:v3:';
