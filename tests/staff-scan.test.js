...
test('parseChkCode：格式正確回內部碼', () => {
  assert.deepEqual(parseChkCode('CHK|nendkai2026|JDC-ABCDEF|abc123', 'nendkai2026'),
    { ok: true, internalId: 'JDC-ABCDEF' });
});

test('parseChkCode：回傳的欄名必須是 internalId，不可以還叫 empNo', () => {
  // 這條擋的不是「值對不對」，是**名字騙人**：值換成內部碼了、欄名還留著叫員編，
  // 下一個讀的人會拿它去跟名冊的員編比對，而那永遠對不上、也不會報錯。
  const r = parseChkCode('CHK|a|JDC-ABCDEF|sig', 'a');
  assert.equal(r.internalId, 'JDC-ABCDEF');
