...
test('中文姓名搜得到，單姓也算', () => {
  assert.deepEqual(ids(S.search(INDEX, '安幼美')), ['b1']);
  assert.deepEqual(ids(S.search(INDEX, '周')), ['b2']);   // 單姓查詢：挑一個「該字在整段 ROWS 只出現一次」的人（2026-09-10 假名化一併更新）
});
...
const FULL = V.groupBatches(H, [row({
  ...
  對象單位: '管理部',
  訊息型別: 'text+image',
  訊息內容: '恭喜你服務屆滿十年。' + '前置贅字'.repeat(20) + '關鍵字在很後面：姚小修領取地點在三樓',
...
test('12 欄全覆蓋：對象姓名', () => { assert.equal(hit('安幼美'), 1); });
...
test('12 欄全覆蓋：訊息內容要收完整的，不是只收畫面上那一行摘要', () => {
  ...
  assert.equal(FULL[0].gist.indexOf('姚小修'), -1, '前提：摘要確實截斷在那個詞之前');
  assert.equal(hit('姚小修'), 1);
});
